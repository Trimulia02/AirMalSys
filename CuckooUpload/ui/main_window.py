...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # <-- DIUBAH: Ukuran awal dipertahankan kecil
    ...
    def switch_to_analysis_progress(self):
        logging.info("MainWindow: Menerima panggilan switch_to_analysis_progress.")
    ...
    def show_report_summary_screen(self):
        try:
            # <-- DIUBAH: Perbesar jendela SEBELUM menampilkan laporan
            self.setFixedSize(720, 460)
            logging.info(f"Jendela diperbesar ke {self.size().width()}x{self.size().height()} untuk laporan.")
    ...
    def switch_to_upload_after_rebuild(self):
        ...
        # <-- DIUBAH: Kecilkan kembali jendela SEBELUM kembali ke upload
        self.setFixedSize(480, 340)
        logging.info(f"Jendela dikembalikan ke {self.size().width()}x{self.size().height()}.")
